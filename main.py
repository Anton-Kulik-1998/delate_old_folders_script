import threading
from app.delete_old_folders_script import check_and_remove_old_files
...
def main(directory_path, days_to_keep, time_to_sleep):
    ...
    thread = threading.Thread(target=check_and_remove_old_files, args=(directory_path, days_to_keep, time_to_sleep))
    thread.daemon = True  # Поток завершится, если основной поток завершится
    thread.start()

if __name__ == "__main__":
    main(FOLDER_PATH, DAYS_TO_KEEP, TIME_TO_SLEEP)
